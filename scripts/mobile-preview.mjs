...
await mkdir(`${OUT}/light`, { recursive: true });
await mkdir(`${OUT}/dark`, { recursive: true });

async function captureTheme(theme) {
  console.log(`\n  Capturing screens in ${theme.toUpperCase()} theme...`);
  ...
  async function shot(name, path, { waitFor, action } = {}) {
    ...
    console.log(`  ✓ [${theme}] ${path}  →  ${OUT}/${theme}/${name}.png`);
  }
  ...
  await page.screenshot({ path: `${OUT}/${theme}/02-sign-in-demo-selected.png` });
  console.log(`  ✓ [${theme}] /auth/sign-in (demo selected)  →  ${OUT}/${theme}/02-sign-in-demo-selected.png`);
  ...
  for (const screen of [
    ...
    await desktopPage.screenshot({ path: `${OUT}/${theme}/${screen.name}.png` });
    console.log(`  ✓ [${theme}] desktop ${screen.path}  →  ${OUT}/${theme}/${screen.name}.png`);
  }
...
console.log(`\nDone — screenshots written to ./${OUT}/light/ and ./${OUT}/dark/`);
